...
class fleet_map_sheet(models.Model):
        ...
        for record in self:
            if record.odometer_start_id:
                ...
                odometer = self.env['fleet.vehicle.odometer'].search(
                    [('vehicle_id', '=', record.vehicle_id.id), ('date', '<=', record.date_start)], limit=1,
                    order='date desc')
                if odometer:
    ...
    odometer_end = fields.Float(compute='_compute_odometer_end', inverse='_set_odometer_end',
                                states={'done': [('readonly', True)]},
                                string='Odometer End',
                                help='Odometer measure of the vehicle at the end of this map sheet')

    odometer_start = fields.Float(compute='_compute_odometer_start', inverse='_set_odometer_start',
                                  states={'done': [('readonly', True)]},
    ...
    def _check_dates(self):
        if self.date_start > self.date_end:
            raise ValidationError(
                _("Map Sheet end-date (%s) must be greater then start-date (%s)") % (self.date_end, self.date_start))
...
class fleet_route_log(models.Model):
    ...
    date_begin = fields.Datetime(string='Date Begin', states={'done': [('readonly', True)]},
                                 default=_get_default_date_begin)
    date_end = fields.Datetime(string='Date End', states={'done': [('readonly', True)]},
                               default=_get_default_date_begin)
    ...
    vehicle_id = fields.Many2one('fleet.vehicle', string='Vehicle',
                                 states={'done': [('readonly', True)]}, )  # default=_get_default_vehicle_id )
    ...
    norm_cons = fields.Float(compute='_compute_dist_echiv', string='Normal Consumption',
    ...
    @api.one
    @api.depends('dist_c1','dist_c2','dist_c3') # daca modifc onsumul la la o masina nu trebuie sa modific toate foile din urma, 'vehicle_id.avg_cons')
    def _compute_dist_echiv(self):
        self.dist_echiv = self.dist_c1 + 1.1 * self.dist_c2 + 1.4 * self.dist_c3
        if self.vehicle_id:
...
##    start < date_begin < end or date_begin < start < date_end
    ...
    def on_change_route(self):
        ...
        if self.route_id and self.date_begin:
            ...
            date_end = date_end + timedelta(hours=int(math.floor(self.route_id.duration)),
